...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", required=True, choices=["pbmc3k", "lung65k", "mouse_brain_1m"])
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use GPU-accelerated DE via rapids-singlecell (requires GPU + rsc installed).",
    )
...
def pipeline_registry(prefix: str) -> dict[str, PipelineFiles]:
    return {
        ...
        "rsc_gpu": PipelineFiles(
            name="rsc_gpu",
            clusters=f"write/{prefix}_rsc_gpu_harmonized_clusters.csv",
            markers=f"write/{prefix}_rsc_gpu_harmonized_markers.csv",
            h5ad=f"write/{prefix}_rsc_gpu_harmonized.h5ad",
            kind="scanpy_like",
        ),
...
def main() -> None:
    ...
    if args.gpu:
        try:
            ...
            print("GPU mode: RMM initialized, using rapids-singlecell for standardized DE")
    ...
    for name in pipelines:
        ...
        print(f"  Standardized DE for {name} ({'GPU' if args.gpu else 'CPU'})...", end=" ", flush=True)
